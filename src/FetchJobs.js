...
function FetchJobs(props) {
  ...
  function generateQuery(filters, pagenum = 0) {
    ...
    if (filters.title !== "") {
      queryPrefixes++;
      query += `${queryPrefixes === 1 ? "?" : "&"}search=${snakeCase(
        filters.title
      )}`;
    }
    ...
    if (filters.full_time) {
      queryPrefixes++;
      query += `${queryPrefixes === 1 ? "?" : "&"}full_time=${
        filters.full_time
      }`;
    }
  ...
  if (error) {
    ...
    return (
      <div>
        <h1 style={{ textAlign: "center" }}>Loading Listings...</h1>
      </div>
    );
  } else {
    return (
      <div style={{ margin: "0 10%" }}>
        <h1 style={{ textAlign: "center" }}>GitHub Jobs is Deprecated...</h1>
        <p>
          Unforunately, GitHub Jobs is officially deprecated, and so is its API.
          I'm currently working on a fix to use some static data for the pages
          instead. However, the code should still very much be viewable and it
          did function when the API was still running. I apologize for the
          inconvenience.
        </p>
      </div>
    );
